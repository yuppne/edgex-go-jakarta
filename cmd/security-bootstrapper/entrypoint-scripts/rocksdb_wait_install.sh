...
# This is customized entrypoint script for RocksDB.
...
echo "Script for waiting security bootstrapping on RocksDB"

# gating on the TokensReadyPort
echo "$(date) Executing waitFor on RocksDB with waiting on TokensReadyPort \
...
# started securely.
# 보안 관련 파일
echo "$(date) ${STAGEGATE_SECRETSTORESETUP_HOST} tokens ready, bootstrapping rocksdb..."
...
echo "$(date) Starting edgex-rocks ..."

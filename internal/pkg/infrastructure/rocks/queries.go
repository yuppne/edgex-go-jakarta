...
func getObjectById(conn rocksClient.Client, id string, out interface{}) errors.EdgeX {
	// obj, err := redis.Bytes(conn.Do(GET, id))

	ro := grocksdb.NewDefaultReadOptions()
	db := conn.Database

	obj, err := db.Get(ro, []byte(id))
...
func getObjectByHash(conn rocksClient.Client, hash string, field string, out interface{}) errors.EdgeX {
...
func getObjectsByRange(conn rocksClient.Client, key string, start, end int) ([][]byte, errors.EdgeX) {
...
func getObjectsByRevRange(conn rocksClient.Client, key string, offset int, limit int) ([][]byte, errors.EdgeX) {
...
func getObjectsBySomeRange(conn rocksClient.Client, command string, key string, offset int, limit int) ([][]byte, errors.EdgeX) {
	if limit == 0 {
		return [][]byte{}, nil
	}
	start := offset
...
func getObjectsByScoreRange(conn rocksClient.Client, key string, start int, end int, offset int, limit int) (objects [][]byte, edgeXerr errors.EdgeX) {
	if limit == 0 {
		return
	}
...
// command (i.e. RANGE, REVRANGE). The entries are retrieved in the order specified by the supplied Redis command.
func getObjectsByLabelsAndSomeRange(conn rocksClient.Client, command string, key string, labels []string, offset int, limit int) ([][]byte, errors.EdgeX) {
...
func getObjectsByIds(conn rocksClient.Client, ids []interface{}) ([][]byte, errors.EdgeX) {
	var result [][]byte
	var err error
	if len(ids) > 0 {
...
func getMemberCountByScoreRange(conn rocksClient.Client, key string, start int, end int) (uint32, errors.EdgeX) {
...
func getMemberCountByLabels(conn rocksClient.Client, command string, key string, labels []string) (uint32, errors.EdgeX) {
...
func getMemberNumber(conn rocksClient.Client, command string, key string) (uint32, errors.EdgeX) {
...
func unionObjectsByKeys(conn rocksClient.Client, offset int, limit int, redisKeys ...string) ([][]byte, errors.EdgeX) {
...
func intersectionObjectsByKeys(conn rocksClient.Client, offset int, limit int, redisKeys ...string) ([][]byte, errors.EdgeX) {
...
func objectsByKeys(conn rocksClient.Client, setMethod string, offset int, limit int, redisKeys ...string) ([][]byte, errors.EdgeX) {
	if limit == 0 {
		return [][]byte{}, nil
	}
	end := offset + limit - 1
...
func unionObjectsByKeysAndScoreRange(conn rocksClient.Client, start, end, offset, limit int, redisKeys ...string) ([][]byte, uint32, errors.EdgeX) {
...
func objectsByKeysAndScoreRange(conn rocksClient.Client, setMethod string, start, end, offset, limit int, redisKeys ...string) (objects [][]byte, totalCount uint32, edgeXerr errors.EdgeX) {
